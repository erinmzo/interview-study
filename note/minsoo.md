...
자바스크립트는 싱글 스레드지만, 브라우저나 Node.js가 제공하는 비동기 처리 시스템 덕분에 여러 작업을 동시에 처리하는 것처럼 보입니다. 비동기 작업이 발생하면 Web API에 위임하고, 작업이 완료되면 태스크 큐에 들어갑니다. 그러면 이벤트 루프가 콜 스택이 비었을 때 이 작업들을 실행하죠. 이런 구조 덕분에 블로킹 없이 여러 작업을 효율적으로 처리할 수 있습니다.

##### 💡예상 꼬리 질문

### ❓태스크 큐의 종류에는 어떤 게 있나요?

크게 매크로태스크 큐와 마이크로태스크 큐가 있습니다. 매크로태스크 큐에는 setTimeout 같은 일반적인 비동기 작업이 들어가고, 마이크로태스크 큐에는 Promise의 then 같은 높은 우선순위 작업이 들어갑니다. 이벤트 루프는 매크로태스크 하나를 처리한 뒤 마이크로태스크 큐를 모두 비운 다음에야 다음 매크로태스크로 넘어가요. 그래서 Promise의 then이 setTimeout보다 항상 먼저 실행되는 겁니다.
