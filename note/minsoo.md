...
> ES6(ECMAScript 2015)는 자바스크립트의 주요 업데이트 버전으로, 많은 새로운 기능과 문법이 추가되었습니다. 주요 특징으로는 let과 const 키워드의 도입, 화살표 함수, 클래스, 모듈 시스템, 템플릿 리터럴, 구조 분해 할당, Promise, 그리고 스프레드 연산자 등이 있습니다.
...
- let은 블록 스코프를 가지는 변수를 선언할 때 사용됩니다.
- const는 재할당이 불가능한 상수를 선언할 때 사용됩니다.

> 2. 화살표 함수

- 함수를 더 간결하게 작성할 수 있게 해주는 문법입니다.
- this 바인딩이 렉시컬 스코프를 따릅니다.

> 3. 클래스

- 객체 지향 프로그래밍을 더 쉽게 구현할 수 있게 해주는 문법적 설탕입니다.
- constructor, extends 등의 키워드를 통해 상속과 인스턴스 생성을 더 명확하게 할 수 있습니다.

> 4. 모듈 시스템

- import와 export 키워드를 통해 코드를 모듈화하고 재사용할 수 있습니다.
- 코드의 의존성을 명확하게 관리할 수 있습니다.

> 5. 템플릿 리터럴

- 백틱(`)을 사용하여 문자열 내에 변수를 쉽게 삽입할 수 있습니다.
- 여러 줄의 문자열을 쉽게 작성할 수 있습니다.

> 6. 구조 분해 할당

- 배열이나 객체의 요소를 쉽게 변수에 할당할 수 있습니다.
- 함수의 매개변수로도 활용할 수 있습니다.
...
> Promise는 비동기 작업의 최종 완료(또는 실패)와 그 결과값을 나타내는 객체입니다. Promise는 비동기 작업이 맞이할 미래의 완료 또는 실패와 그 결과값을 나타냅니다. Promise는 세 가지 상태를 가집니다: 대기(pending), 이행(fulfilled), 거부(rejected).
...
- then()을 통해 처리 결과값을 받을 수 있음

> 3. 거부(Rejected)

- 비동기 처리가 실패하거나 오류가 발생한 상태
- catch()를 통해 실패한 이유(에러)를 받을 수 있음

### ❓Promise와 콜백의 차이점은 무엇인가요?

> 1. 에러 처리

- Promise는 catch를 통해 체계적인 에러 처리가 가능합니다.
- 콜백은 try-catch로 에러를 잡을 수 없어 에러 처리가 어렵습니다.

> 2. 코드 가독성

- Promise는 체이닝을 통해 가독성 있게 코드를 작성할 수 있습니다.
- 콜백은 중첩될 경우 콜백 헬이 발생하여 가독성이 떨어집니다.

> 3. 비동기 처리 순서

- Promise는 then을 통해 명확하게 실행 순서를 보장받을 수 있습니다.
- 콜백은 여러 비동기 작업의 순서를 보장하기 어렵습니다.

### ❓useEffect가 호출되는 시점에 대해 설명해 주세요.

> useEffect는 컴포넌트의 렌더링이 완료된 후에 호출됩니다. 구체적으로는 컴포넌트가 마운트 될 때, 업데이트 될 때, 그리고 언마운트 될 때 호출될 수 있습니다. useEffect는 클린업 함수를 통해 사이드 이펙트를 정리할 수 있습니다.
...
- 특정 값을 포함한 배열([value])을 전달하면 해당 값이 변경될 때마다 실행됩니다.
- 의존성 배열을 생략하면 모든 렌더링 후에 실행됩니다.

### ❓클린업 함수는 언제 사용해야 하나요?

> 클린업 함수는 주로 다음과 같은 상황에서 사용됩니다:

- 이벤트 리스너를 제거할 때
- WebSocket 연결을 종료할 때
- setInterval이나 setTimeout을 정리할 때
- API 요청을 취소할 때

클린업 함수는 컴포넌트가 언마운트되거나, 다음 useEffect가 실행되기 전에 호출됩니다.

### ❓JWT(JSON Web Token)에 대해 설명해주세요.

> JWT는 당사자 간의 정보를 JSON 객체로 안전하게 전송하기 위한 컴팩트하고 독립적인 방식의 웹 토큰입니다. JWT는 Header, Payload, Signature 세 부분으로 구성되며, 각 부분은 점(.)으로 구분됩니다. 주로 사용자 인증과 정보 교환에 사용됩니다.
...
- 토큰의 타입과 해시 암호화 알고리즘(ex: HMAC SHA256, RSA)을 명시합니다.
- 일반적으로 Base64Url로 인코딩됩니다.

> Payload(페이로드)

- 토큰에 담을 정보가 들어있습니다.
- 사용자 ID, 권한, 만료시간 등의 클레임(claim) 정보를 포함합니다.
- 마찬가지로 Base64Url로 인코딩됩니다.

> Signature(서명)

- 헤더와 페이로드를 기반으로 비밀키를 사용하여 생성됩니다.
- 토큰이 변조되지 않았음을 확인하는데 사용됩니다.

### ❓JWT의 장단점은 무엇인가요?

> 장점:

- 서버의 세션 저장소가 필요 없어 서버 자원을 절약할 수 있습니다.
- 클라이언트 측에서 토큰을 저장하고 관리합니다.
- 확장성이 좋고 서버 간 공유가 용이합니다.
- REST API에서 사용하기 적합합니다.

> 단점:

- 토큰 크기가 세션 ID에 비해 상대적으로 큽니다.
- 한번 발급된 토큰은 만료되기 전까지 계속 사용 가능하므로, 토큰이 탈취되면 보안에 취약할 수 있습니다.
- Payload는 단순 인코딩되어 있어 중요한 정보는 담지 않아야 합니다.

### ❓JWT는 어디에 저장하는 것이 좋을까요?

> 브라우저의 경우:

- localStorage: XSS 공격에 취약할 수 있습니다.
- httpOnly 쿠키: CSRF 공격 가능성이 있지만, XSS 공격으로부터는 안전합니다.
- 보안을 위해서는 httpOnly 속성이 설정된 쿠키에 저장하는 것이 권장됩니다.

### 모바일 앱의 경우:

> 안전한 저장소(iOS의 Keychain, Android의 Keystore)에 저장하는 것이 좋습니다.
