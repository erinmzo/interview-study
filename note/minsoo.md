...
쉽게 말해서, 함수가 자신이 생성될 당시의 스코프를 기억하고 있다가 나중에 자신이 선언된 곳이 아닌 다른 곳에서 호출되어도 그 스코프에 접근할 수 있게 해주는 것이죠.
...
2. event.preventDefault()를 사용하는 방법인데요, 이는 이벤트의 기본 동작 자체를 취소합니다. 폼 제출이나 링크 클릭 같은 브라우저의 기본 동작을 막고 싶을 때 사용하죠.
...
또한 레이지 로딩 기법을 사용하면 이미지나 비디오 같은 무거운 리소스들을 실제로 화면에 표시될 때만 로드하도록 할 수 있습니다. 이는 초기 로딩 시간을 크게 단축시킬 수 있죠.
...
1. 이미지 포맷을 최적화할 수 있습니다. WebP나 AVIF 같은 최신 포맷을 사용하면 화질은 유지하면서도 파일 크기를 크게 줄일 수 있습니다. 다만 브라우저 호환성을 고려해야 하죠.
...
3. 클래스 문법의 도입으로 객체 지향 프로그래밍이 더 직관적으로 변했습니다. 생성자, 상속, 메서드 정의 등을 더 명확하게 표현할 수 있게 되었죠.
...
여기서 사이드 이펙트란 API 호출이나 구독 설정, DOM 조작 같은 작업들을 말하는데요, 이런 작업들은 렌더링 과정에 영향을 미칠 수 있기 때문에 렌더링이 완료된 후에 수행하는 것이 좋습니다.

useEffect의 실행 시점은 의존성 배열을 어떻게 설정하느냐에 따라 달라지는데요, 빈 배열을 전달하면 컴포넌트가 처음 마운트될 때만 실행되고, 특정 값을 배열에 넣으면 그 값이 변경될 때마다 실행됩니다.
의존성 배열을 아예 생략하면 모든 렌더링 후에 실행되죠.

재미있는 점은 useEffect에서 함수를 반환할 수 있다는 건데요, 이를 클린업 함수라고 합니다.
이 함수는 컴포넌트가 화면에서 사라지거나, 다음 useEffect가 실행되기 전에 호출되어요. 예를 들어 이벤트 리스너를 등록했다면, 클린업 함수에서 이를 제거해주는 식이죠.

실제로 많이 사용되는 예시를 들어보면, 컴포넌트가 마운트될 때 서버에서 데이터를 가져온다거나, 웹소켓 연결을 설정하거나, 또는 스크롤 이벤트 리스너를 등록하는 경우가 있습니다.
이때 클린업 함수에서는 웹소켓 연결을 종료하거나 이벤트 리스너를 제거하는 등의 정리 작업을 수행하게 되죠.

이렇게 useEffect를 잘 활용하면 컴포넌트의 생명주기에 따른 다양한 작업들을 효과적으로 관리할 수 있습니다.
...
1. XSS(Cross-Site Scripting) 공격이 있습니다. 이는 악성 스크립트를 웹 페이지에 삽입하여 실행하는 공격인데요, 이를 방지하기 위해서는 모든 사용자 입력값을 적절히 이스케이프 처리해야 합니다. React를 사용할 때는 JSX가 기본적으로 이스케이프 처리를 해주지만, dangerouslySetInnerHTML 같은 기능을 사용할 때는 특별히 주의해야 합니다.
